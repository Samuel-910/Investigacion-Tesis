...
@Component({
    ...
    templateUrl: './nota-credito-list.component.html'
})
export class NotaCreditoListComponent implements OnInit {
    ventas = signal<any[]>([]);
    ...
    imprimir(venta: any): void {
        ...
        this.puntoDocumentoService.obtenerPorPunto(authPuntoId).subscribe({
            next: (res) => {
                if (res.success && res.data) {
                    let asignacion = res.data.find(a => 
                        a.modulo === moduleType && 
                        a.estado === 'ACTIVO' &&
    ...
    canjearNotaVenta(venta: any): void {
        // Redirige al registro de venta pasando el ID de la Nota de Venta para canjear
        this.router.navigate(['/ventas/venta'], { queryParams: { canjearId: venta.idVenta } });
    }
