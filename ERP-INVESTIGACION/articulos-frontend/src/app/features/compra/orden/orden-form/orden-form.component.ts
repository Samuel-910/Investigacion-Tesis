import { Component, OnInit, signal, computed, HostListener, ElementRef, Input, OnChanges, SimpleChanges, inject, ViewChildren, QueryList } from '@angular/core';
...
export class OrdenFormComponent implements OnInit, OnChanges {
    ...
    esGrupoNuevo = signal(true);
    isGroupLocked = signal(false);
    grupoOptions = computed(() => this.grupos().map(g => ({ label: g, value: g })));
    proveedoresBloqueados = signal<number[]>([]);
    proveedoresFiltrados = computed(() => {
        const bloqueados = this.proveedoresBloqueados();
        return this.proveedores().filter(p => !bloqueados.includes(p.id));
    });
    ...
    historialPreciosMap = new Map<number, any[]>(); // idCatalogo -> precioMaximo (historico)
    ...
    // Map to store unit names by ID for quick lookup
    unidadesMap = new Map<number, string>();
    // Array parallel to details FormArray to store unit options for each row
    rowUnitOptions = signal<any[][]>([]);

    @HostListener('document:click', ['$event'])
    onClickOutside(event: Event) {
        const targetElement = event.target as HTMLElement;
        const searchContainer = this.elementRef.nativeElement.querySelector('.search-container-wrapper');
    ...
    onBlurSearch() {
        // Delay slighty to allow click on dropdown items to register
        setTimeout(() => {
            this.showDropdownProducto.set(false);
        }, 200);
    }
    ...
    cargarGrupos() {
        ...
        this.compraService.listarGrupos(idSucursal, 0, 10).subscribe(res => {
    ...
    onGrupoExistenteSelected(obj: any) {
        const nombre = obj.value;
        this.form.get('nombreGrupo')?.setValue(nombre);
        this.cargarProductosDeGrupo(nombre);

        const idSucursal = this.authService.getSucursalIdFromToken();
        if (idSucursal) {
            this.compraService.obtenerProveedoresPorGrupo(nombre, idSucursal).subscribe(res => {
                if (res.success && res.data) {
                    const dataArray = Array.isArray(res.data) ? res.data : ((res.data as any).content || []);
                    this.proveedoresBloqueados.set(dataArray);
                    
                    const currentProv = this.form.get('idProveedor')?.value;
                    if (currentProv && dataArray.includes(currentProv)) {
                        this.form.get('idProveedor')?.setValue(null);
                        this.alertService.error('Proveedor no disponible', 'Este proveedor ya fue agregado en este grupo de compra.');
                    }
                }
            });
        }
    ...
    cargarProductosDeGrupo(nombre: string) {
        const idSucursal = this.authService.getSucursalIdFromToken();
        if (!idSucursal) return;

        this.loading.set(true);
        this.compraService.obtenerProductosPorGrupo(nombre, idSucursal).subscribe(res => {
            this.loading.set(false);
            
            let dataArray: any[] = [];
            if (res.success && res.data) {
                dataArray = Array.isArray(res.data) ? res.data : ((res.data as any).content || []);
            }
            ...
            if (dataArray.length > 0) {
                ...
                // Add products from group with their original quantities
                dataArray.forEach((d: any) => {
                    const prod = d.producto;
                    if (prod) {
                        // Pasamos el producto y la cantidad original del detalle
                        this.agregarDetalle(prod, d.cantidad);

                        // Ajustamos la unidad de medida si viene en el detalle
                        const lastIndex = this.detalles.length - 1;
                        if (d.idUnidadMedida) {
                            const row = this.detalles.at(lastIndex);
                            row.patchValue({
                ...
                this.isGroupLocked.set(true);
                this.alertService.toast('Productos y cantidades bloqueados por grupo', 'info');
                this.syncLockedControls();
            } else {
                this.isGroupLocked.set(false);
            }
        });
    }

    cambiarModoGrupo(esNuevo: boolean) {
        this.esGrupoNuevo.set(esNuevo);
        this.isGroupLocked.set(false);
        this.proveedoresBloqueados.set([]);
        if (esNuevo) {
            this.form.get('nombreGrupo')?.setValue('');
        }
        this.syncLockedControls(); // Re-habilitar si es nuevo
    }

    syncLockedControls() {
        const locked = this.isGroupLocked();
        this.detalles.controls.forEach(ctrl => {
            const row = ctrl as FormGroup;
            if (locked) {
                row.get('cantidad')?.disable({ emitEvent: false });
                row.get('idUnidadMedida')?.disable({ emitEvent: false });
            } else {
                row.get('cantidad')?.enable({ emitEvent: false });
                row.get('idUnidadMedida')?.enable({ emitEvent: false });
            }
        });
    }
    ...
    cargarCompra(id: number) {
        ...
        this.compraService.obtener(id).subscribe({
            next: (res) => {
                ...
                if (res.success && res.data) {
                    ...
                    this.form.patchValue({
                        ...
                        valorVentaGravado: data.valorVentaGravado,
                        valorVentaExonerado: data.valorVentaExonerado,
                        valorVentaInafecto: data.valorVentaInafecto,
                        igv: data.igv,
                        total: data.total
                    });
                    ...
                    this.rowUnitOptions.set([]); // Limpiar opciones de unidades para evitar desfases

                    data.detalles.forEach((d: any) => {
                        const prodId = Number(d.producto?.id);
                        if (prodId) {
                            // Primero solicitamos datos completos para asegurar los factores de conversión y niveles
                            this.catalogoService.obtenerPorId(prodId).subscribe(resProd => {
                                if (resProd.success) {
                                    const fullProd = resProd.data;
                                    this.productDataMap.update(map => {
                                        map.set(prodId, fullProd);
                                        return new Map(map);
                                    });

                                    // Lógica unificada: agregar la fila con los datos del catálogo ya cargados
                                    this.agregarDetalle(fullProd, d.cantidad);

                                    // SOBRESCRIBIR con los valores específicos que se guardaron en la compra
                                    ...
                                    row.patchValue({
                                        ...
                                        manejaLotes: fullProd.manejaLotes ?? true,
                                        lote: d.lote,
                                        ...
                                        presentacion: fullProd.presentacion || ''
                                    });
                                }
                            });
                        }
            ...
            complete: () => {
                if (this.soloResumen) {
                    setTimeout(() => this.deshabilitarParaSoloResumen(), 100);
                } else if (this.idCompra) {
                    setTimeout(() => this.deshabilitarParaEdicion(), 200);
                }
            }
        });
    }
    ...
    deshabilitarParaEdicion() {
        const camposPermitidos = ['idProveedor', 'detalles', 'percepcion', 'ajusteRedondeo'];
        ...
        this.detalles.controls.forEach(control => {
            Object.keys((control as FormGroup).controls).forEach(key => {
                if (!['precioUnitario', 'porcentajeDescuento', 'porcentajeDescuento2'].includes(key)) {
                    control.get(key)?.disable({ emitEvent: false });
                }
            });
        });
    }
    ...
    initForm() {
        this.form = this.fb.group({
            ...
            igv: [0],
            total: [0],
            nombreGrupo: ['', (control: any) => {
                if (!control.value || control.value.toString().trim() === '') {
                    return { required: true };
                }
                if (this.esGrupoNuevo() && this.grupos().includes(control.value)) {
                    return { duplicatedGroup: true };
                }
                return null;
            }],
        ...
        // Observar cambios en campos manuales (solo efectivo en modo soloResumen)
        const manualFields = ['valorVentaGravado', 'valorVentaExonerado', 'valorVentaInafecto', 'igv'];
        manualFields.forEach(field => {
            this.form.get(field)?.valueChanges.subscribe(() => {
                if (this.soloResumen) this.calculateTotals();
            });
        });
    }
    ...
    agregarDetalle(producto: any = null, cantidad: number | null = null) {
        ...
        const detalle = this.fb.group({
            ...
            lote: [''],
            fechaVencimiento: [''],
            tipoAfectacion: [(producto?.tipoAfectacion || '').trim() || '10', Validators.required],
        ...
        // Descuentos: maximo 100 y auto-activar bonificación
        ['porcentajeDescuento', 'porcentajeDescuento2'].forEach(campo => {
            detalle.get(campo)?.valueChanges.subscribe(val => {
                let currentVal = val;
                if (currentVal > 100) {
                    currentVal = 100;
                    setTimeout(() => detalle.get(campo)?.setValue(100, { emitEvent: false }));
                }
                if (currentVal === 100) {
                    setTimeout(() => detalle.get('esBonificacion')?.setValue(true));
                }
                this.calculateTotals();
            });
        });
            ...
            if (index >= 0) {
                ...
                if (opt) {
                    ...
                    if (currentPrice > 0 && oldFactor > 0 && newFactor > 0 && oldFactor !== newFactor) {
                        ...
                        (detalle.get('precioUnitario') as any)?.setValue(Number(newPrice.toFixed(4)));
                    }
                }
            }
        });
        ...
        // Fetch max price from history if product exists
        if (producto?.id) {
            this.compraService.obtenerPrecioMaximoHistorico(producto.id).subscribe(res => {
                if (res.success) {
                    this.maxPreciosMap.set(producto.id, res.data);
                    this.validarMaximo(detalle);
                }
            });
            this.compraService.obtenerHistorialPorProducto(producto.id).subscribe(res => {
                if (res.success) {
                    const results = (res.data as any).content ? (res.data as any).content : res.data;
                    this.historialPreciosMap.set(producto.id, results);
                }
            });
        }
    }
    ...
    getDesgloseTooltip(index: number): { estimacion: any[], catalogo: any[] } {
        ...
        // 3. HISTORIAL DE PRECIOS (Últimas 3 compras)
        const historial = this.historialPreciosMap.get(productoId) || [];
        const historialTop = historial.slice(0, 3).map(h => {
            const dateStr = h.fechaEmision ? new Date(h.fechaEmision).toLocaleDateString() : '';
            return { 
                nombre: `${h.presentacion || 'Unid.'} (${dateStr})`, 
                precio: Number(h.precioUnitario).toFixed(2) 
            };
        });

        return { estimacion, catalogo: historialTop };
    }
    ...
    navegarTeclado(event: KeyboardEvent, i: number, col: number) {
        const key = event.key;
        const totalCols = 4; // Cantidad, Precio, Dcto1, Dcto2
        let nextRow = i;
        let nextCol = col;
    ...
    calculateTotals() {
        ...
        if (this.soloResumen) {
            gravada = Number(this.form.get('valorVentaGravado')?.value) || 0;
            exonerada = Number(this.form.get('valorVentaExonerado')?.value) || 0;
            inafecta = Number(this.form.get('valorVentaInafecto')?.value) || 0;
            igv = Number(this.form.get('igv')?.value) || 0;
            ...
            this.detalles.controls.forEach(ctrl => {
                ...
                // Cǭlculo de valor neto paso a paso (PRECIO DIRECO DE LA FILA)
                ...
                if (tipo === '10' || tipo === 'GRAVADO') {
                    ...
                    // El total de la fila sigo lo que el usuario quiere ver (Con o Sin IGV)
                    const totalVisibleItem = incluyeIgv ? Number((valorNeto + igvItem).toFixed(2)) : valorNeto;

                    gravada += valorNeto;
                    igv += igvItem; // El IGV global SIEMPRE se suma para el reporte
                    ...
                    const igvItem = Number((valorNeto * 0.18).toFixed(2));
                    const totalItem = Number((valorNeto + igvItem).toFixed(2));
                    gravada += valorNeto;
                    igv += igvItem;
                    ctrl.get('totalItem')?.setValue(totalItem, { emitEvent: false });
                }
            });
    ...
    onProductoSelected(producto: any, cantidad: number = 1) {
        if (producto) {
            ...
            if (!producto.niveles || producto.niveles.length === 0) {
                this.catalogoService.obtenerPorId(prodId).subscribe(res => {
                    if (res.success) {
                        ...
                        this.agregarDetalle(fullProd, cantidad);
    ...
    onSubmit() {
        ...
        const req = {
            ...formValue,
            idSucursal,
            isAjusteManual: this.soloResumen
        };

        const request$ = this.idCompra
            ? this.compraService.actualizarOrden(this.idCompra, req as any)
            : this.compraService.crearOrden(req as any);

        request$.subscribe({
            next: (res) => {
                if (res.success) {
                    this.alertService.toast(this.idCompra ? 'Compra actualizada' : 'Orden creada exitosamente', 'success');
                    this.guardado.emit();
                    this.resetForm();
                    this.cargarGrupos();
                }
